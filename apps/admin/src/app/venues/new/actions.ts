...
type ActionResult = { ok: true; id: string; name: string } | { ok: false; error: string };
...
export async function createVenueAction(input: CreateVenueInput): Promise<ActionResult> {
  ...
  if (!res.ok) {
    ...
    const error = Array.isArray(body.message)
      ? body.message.join(", ")
      : (body.message ?? "Failed to create venue.");
    return { ok: false, error };
  }

  const venue = (await res.json()) as { id: string; name: string };
  return { ok: true, id: venue.id, name: venue.name };
}
