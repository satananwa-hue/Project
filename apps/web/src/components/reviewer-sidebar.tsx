...
export function ReviewerSidebar({ profile }: { profile: UserProfile }) {
  ...
  return (
    <>
      {/* Avatar trigger in the header top-right */}
      <button
        onClick={() => setOpen(true)}
        aria-label="Open profile"
        className="fixed top-3 right-5 z-50 h-10 w-10 overflow-hidden rounded-full border-2 border-accent/50 bg-surface hover:border-accent transition-colors shadow-md"
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={`/levels/level_${level}.png`}
          alt={levelName}
      ...
      {/* Bottom sheet */}
      <div
        className={`fixed bottom-0 left-0 right-0 z-50 max-h-[90vh] overflow-y-auto rounded-t-2xl bg-surface border-t border-border shadow-2xl transition-transform duration-300 ${
        ...
        <div className="px-5 pb-10 pt-4">
          ...
          <div className="mt-1 flex flex-col">
            <Link
              href="/reviews/new"
              onClick={() => setOpen(false)}
              className="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm hover:bg-surface-raised transition-colors"
            >
              <span className="text-base">✏️</span>
