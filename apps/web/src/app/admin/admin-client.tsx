...
function PasswordModal({ account, onClose }: { account: AccountDto; onClose: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    await api('POST', `accounts/${account.id}/reset-password`, { newPassword: pw });
    setBusy(false);
    setDone(true);
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl border border-border/40 bg-surface p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        {done ? (
          ...
          <form onSubmit={submit} className="flex flex-col gap-3">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button type="submit" disabled={pw.length < 8 || busy}
...
function UsersTab({ accounts, currentUserId, onRefresh }: { accounts: AccountDto[]; currentUserId: string; onRefresh: () => void }) {
  ...
  async function changeRole(id: string, role: 'CREATOR' | 'ADMINISTRATOR') {
    setBusy(true);
    await api('PATCH', `accounts/${id}`, { role });
    setBusy(false);
    onRefresh();
  }
  ...
  async function toggleActive(id: string, active: boolean) {
    setBusy(true);
    await api('PATCH', `accounts/${id}`, { active });
    setBusy(false);
    onRefresh();
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      {resetTarget && <PasswordModal account={resetTarget} onClose={() => setResetTarget(null)} />}
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={onRefresh} disabled={busy}
          className="text-xs text-muted hover:text-accent transition-colors font-medium disabled:opacity-40">
          ↺ Refresh
        </button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-border/30">
              ...
              <tr key={acc.id} className="border-b border-border/20 hover:bg-surface/40 transition-colors">
                ...
                <td className="px-4 py-3">
                  {acc.id === currentUserId ? (
                    ...
                    <select defaultValue={acc.role === 'USER' ? '' : acc.role}
                      onChange={e => { const v = e.target.value as 'CREATOR' | 'ADMINISTRATOR'; if (v) changeRole(acc.id, v); }}
                ...
                <td className="px-4 py-3">
                  <button onClick={() => toggleActive(acc.id, !acc.active)} disabled={acc.id === currentUserId || busy}
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <button onClick={() => setResetTarget(acc)} className="text-xs text-muted hover:text-accent transition-colors font-medium">
                    Reset PW
                  </button>
                </td>
              </tr>
        ...
        {filtered.length === 0 && <p className="text-center text-muted text-sm py-8">No accounts match.</p>}
      </div>
    </div>
  );
}

function InvitesTab({ invites, onRefresh }: { invites: InviteDto[]; onRefresh: () => void }) {
  ...
  async function handleCreate() {
    setBusy(true);
    await api('POST', 'invites', note ? { note } : {});
    setBusy(false);
    setNote('');
    onRefresh();
  }
  ...
  async function handleBulk() {
    ...
    if (!n || n < 1) return;
    setBusy(true);
    await api('POST', 'invites/bulk', { count: n });
    setBusy(false);
    onRefresh();
  }
    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-end gap-3 p-4 rounded-xl border border-border/30 bg-surface/40">
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex gap-2">
            <input type="number" min={1} max={1000} value={bulk} onChange={e => setBulk(e.target.value)}
      ...
      <div className="overflow-x-auto rounded-xl border border-border/30">
            ...
            {invites.map(inv => (
              <tr key={inv.id} className={`border-b border-border/20 transition-colors ${inv.usedAt ? 'opacity-50' : 'hover:bg-surface/40'}`}>
                <td className="px-4 py-3 font-mono text-xs tracking-wider text-accent">
                  <button onClick={() => copyCode(inv.code)} className="hover:underline">{inv.code}</button>
                ...
                <td className="px-4 py-3">
                  {!inv.usedAt && (
                    <button onClick={() => handleRevoke(inv.code)} disabled={busy}
        ...
        {invites.length === 0 && <p className="text-center text-muted text-sm py-8">No invites yet.</p>}
      </div>
    </div>
  );
}
...
export function AdminClient({ currentUserId }: { currentUserId: string }) {
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      const [statsRes, accountsRes, invitesRes] = await Promise.all([
        fetch('/api/admin/stats'),
        fetch('/api/admin/accounts'),
        fetch('/api/admin/invites'),
      ]);
      if (!statsRes.ok || !accountsRes.ok || !invitesRes.ok) {
        throw new Error('API returned an error — check your session');
      }
      const [stats, accounts, invites] = await Promise.all([
        statsRes.json() as Promise<Stats>,
        accountsRes.json() as Promise<AccountDto[]>,
        invitesRes.json() as Promise<InviteDto[]>,
      ]);
      setData({ stats, accounts, invites });
    } catch (e) {
  ...
  const tabs: { id: Tab; label: string }[] = [
    ...
    { id: 'users', label: data ? `Users (${data.accounts.length})` : 'Users' },
    { id: 'invites', label: data ? `Invites (${data.invites.length})` : 'Invites' },
  ];
      ...
      <div className="flex-1 p-6">
        {error && (
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            ...
            <button onClick={load} className="px-5 py-2 rounded-lg bg-accent text-white text-sm font-bold hover:bg-accent/90 transition-colors">
              Retry
            </button>
          </div>
        )}
        ...
        {!error && data && (
          <>
            {tab === 'overview' && (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                <StatCard label="Total users" value={data.stats.accounts} />
                <StatCard label="Total venues" value={data.stats.venues} sub={`${data.stats.pendingVenues} pending`} />
                <StatCard label="Published" value={data.stats.publishedVenues} />
                <StatCard label="Reviews" value={data.stats.reviews} />
                <StatCard label="Invites" value={data.invites.length} sub={`${data.invites.filter(i => !i.usedAt).length} available`} />
              </div>
            )}
            {tab === 'users' && (
              <UsersTab accounts={data.accounts} currentUserId={currentUserId} onRefresh={load} />
            )}
            {tab === 'invites' && (
              <InvitesTab invites={data.invites} onRefresh={load} />
            )}
          </>
        )}
      </div>
